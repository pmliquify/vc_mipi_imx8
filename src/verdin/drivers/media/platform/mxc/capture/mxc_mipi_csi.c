...
/*----------------------------------------------------------------------*/
static struct csi_state *vc_mipi_csi_state  = NULL;

/****** csi_num_lanes = Get number of CSI lanes = 01.2021 ***************/
int csi_num_lanes(void)
{

#define TRACE_CSI_NUM_LANES         0   /* DDD - csi_num_lanes - trace */
...
#define TRACE_MIPI_CSIS_CLK_GET     0   /* DDD - mipi_csis_clk_get - trace */
...
static int mipi_csis_init_controls(struct csi_state *state)
{
    struct v4l2_ctrl_handler *ctrl_hdlr;
    int ret;

    ctrl_hdlr = &state->ctrl_handler;
    ret = v4l2_ctrl_handler_init(ctrl_hdlr, 1);
    if (ret)
        return ret;

    /* Really not sure about locking... enabling it blindly causes dead locks */
    /* ctrl_hdlr->lock = &state->lock; */

    /*
     * An empty control handler, merely a pass-through.  This allows to expose
     * the subdev (sensor) control handler also via this subdev.
     */

    state->mipi_sd.ctrl_handler = ctrl_hdlr;
    state->v4l2_dev.ctrl_handler = ctrl_hdlr;

    return 0;
}

static void mipi_csis_free_controls(struct csi_state *state)
{
...
#define TRACE_MIPI_CSIS_QUERYCTRL       0   /* DDD - mipi_csis_queryctrl - trace */
...
static int mipi_csis_queryextctrl(struct v4l2_subdev *mipi_sd, struct v4l2_query_ext_ctrl *qec)
{
#define TRACE_MIPI_CSIS_QUERYEXTCTRL    0   /* DDD - mipi_csis_queryextctrl - trace */
...
/****** mipi_csis_s_stream = Start stream = 11.2020 *********************/
static int mipi_csis_s_stream(struct v4l2_subdev *mipi_sd, int enable)
{

#define TRACE_MIPI_CSIS_S_STREAM        0   /* DDD - mipi_csis_s_stream - trace */
#define MIPI_CSIS_S_STREAM_DELAY    30 // 30  /* CCC - mipi_csis_s_stream - time delay in ms: 0=off */
...
static struct v4l2_subdev_core_ops mipi_csis_core_ops = {
    ...
    .queryextctrl  = mipi_csis_queryextctrl,
...
#define TRACE_MIPI_CSIS_IRQ_HANDLER     0   /* DDD - mipi_csis_irq_handler - trace */
...
#define TRACE_MIPI_CSIS_PARSE_DT    0   /* DDD - mipi_csis_parse_dt - trace */
...
{
#define TRACE_MIPI_CSIS_PROBE       0   /* DDD - mipi_csis_probe - trace */
