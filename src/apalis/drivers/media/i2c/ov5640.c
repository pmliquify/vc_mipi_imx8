 ...
 */
#define TRACE printk("        TRACE [vc-mipi] i2c/oc5640.c --->  %s : %d", __FUNCTION__, __LINE__);
...
static int ov5640_init_slave_id(struct ov5640_dev *sensor)
{
	...
	if (ret < 0) {
		dev_err(&client->dev, "[vc-mipi ov5640] %s: failed with %d\n", __func__, ret);
		return ret;
	}
				   ...
				   enum ov5640_frame_rate rate)
{
	...
	case OV5640_MODE_QSXGA_2592_1944:
		...
		break;
	default:
		dev_err(&client->dev, "[vc-mipi ov5640] Invalid mode (%d)\n", mode->id);
		ret = -EINVAL;
	}
...
static int ov5640_set_virtual_channel(struct ov5640_dev *sensor)
{
	...
	if (channel > 3) {
		dev_err(&client->dev,
			"[vc-mipi ov5640] %s: wrong virtual_channel parameter, expected (0..3), got %d\n",
			__func__, channel);
		return -EINVAL;
	}
...
static int ov5640_set_power_on(struct ov5640_dev *sensor)
{
	...
	if (ret) {
		dev_err(&client->dev, "[vc-mipi ov5640] %s: failed to enable clock\n",
			__func__);
		return ret;
	}
	...
	ret = regulator_bulk_enable(OV5640_NUM_SUPPLIES,
				    sensor->supplies);
	if (ret) {
		dev_err(&client->dev, "[vc-mipi ov5640] %s: failed to enable regulators\n",
			__func__);
		goto xclk_off;
	}
	...
	struct v4l2_subdev_frame_interval_enum *fie)
{
	...
	if (fie->width == 0 || fie->height == 0 || fie->code == 0) {
		pr_warn("[vc-mipi ov5640] Please assign pixel format, width and height.\n");
		return -EINVAL;
	}
...
static int ov5640_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	if (sensor->streaming == !enable) {
		ret = ov5640_check_valid_mode(sensor,
					      ...
					      sensor->current_fr);
		if (ret) {
			dev_err(&client->dev, "[vc-mipi ov5640] Not support WxH@fps=%dx%d@%d\n",
...
static int ov5640_check_chip_id(struct ov5640_dev *sensor)
{
	...
	if (ret) {
		dev_err(&client->dev, "[vc-mipi ov5640] %s: failed to read chip identifier\n",
			__func__);
		goto power_off;
	}

	if (chip_id != 0x5640) {
		dev_err(&client->dev, "[vc-mipi ov5640] %s: wrong chip identifier, expected 0x5640, got 0x%x\n",
			__func__, chip_id);
		ret = -ENXIO;
	}
...
static int ov5640_probe(struct i2c_client *client)
{
	...
	if (!ret) {
		...
		case 0:
			break;
		default:
			dev_warn(dev, "[vc-mipi ov5640] %u degrees rotation is not supported, ignoring...\n",
				 rotation);
		}
	}
	...
	endpoint = fwnode_graph_get_next_endpoint(dev_fwnode(&client->dev),
						  NULL);
	if (!endpoint) {
		dev_err(dev, "[vc-mipi ov5640] endpoint node not found\n");
		return -EINVAL;
	}
	...
	if (ret) {
		dev_err(dev, "[vc-mipi ov5640] Could not parse endpoint\n");
		return ret;
	}
	...
	if (IS_ERR(sensor->xclk)) {
		dev_err(dev, "[vc-mipi ov5640] failed to get xclk\n");
	...
	if (sensor->xclk_freq < OV5640_XCLK_MIN ||
		...
		dev_err(dev, "[vc-mipi ov5640] xclk frequency out of range: %d Hz\n",
			sensor->xclk_freq);
		return -EINVAL;
	}
